...
class WATrainer(Trainer):
    ...
    def train(self, dataloader, epoch=0, adversarial=False, verbose=True):
        ...
        for data in tqdm(dataloader, desc='Epoch {}: '.format(epoch), disable=not verbose):
            ...
            if adversarial:
                ...
                elif self.params.beta is not None and self.params.rho is not None:
                    loss, batch_metrics = self.trades_loss(x, y, beta=self.params.beta)
                    loss = loss + self.params.rho * self.prox_term()/2
                elif self.params.rho is not None:
    ...
    def prox_term(self):
        prox = 0
        for param1, param2 in zip(self.wa_model.parameters(),self.model.parameters()):
